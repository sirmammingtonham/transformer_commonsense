...
def main():
    ...
    train_valid = loaded_data.train_test_split(test_size=0.2, seed=201)
    ...
    with training_args.strategy.scope():
        ...
        if tf_data["test"] is not None:
            ...
            loss, accuracy = model.evaluate(tf_data["test"])
            logger.info(f"Loss: {loss:.5f}, Accuracy: {accuracy * 100:.4f}%")
            output = model.predict(tf_data["test"])
            ...
            with open(output_test_file, "w") as writer:
                ...
                for index, (item, label) in enumerate(zip(predicted_class, tf_data["test"]["labels"])):
                    if is_regression:
